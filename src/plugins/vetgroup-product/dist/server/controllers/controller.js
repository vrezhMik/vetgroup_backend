...
const controller = ({ strapi }) => ({
    ...
    async uploadFile(ctx) {
        try {
            ...
            const allCategories = await strapi.entityService.findMany("api::category.category", { fields: ["id", "title"] });
            const categoryMap = new Map(allCategories.map((cat) => [cat.title.trim(), cat.id]));
            const createdProducts = await Promise.all(uniqueProducts.map((product) => {
