...
const controller = ({ strapi: strapi2 }) => ({
  ...
  async uploadFile(ctx) {
    try {
      ...
      await strapi2.db.query("api::product.product").deleteMany({
        where: {}
      });
      ...
      const allCategories = await strapi2.entityService.findMany(
      ...
      const createdProducts = await Promise.all(
        uniqueProducts.map((product) => {
          const matchedCategoryId = categoryMap.get(product.name.trim()) ?? null;
          return strapi2.entityService.create("api::product.product", {
            data: {
...
function readCsvFile(fileBuffer) {
  try {
    ...
    const transformedData = jsonData.map((row, index2) => {
      ...
      if (typeof rawCode === "string" && rawCode.includes(". ") && !rawCode.match(/^\d+$/)) {
        previousValidName = rawCode;
        return null;
      }
      if (!previousValidName && index2 === 0) {
        previousValidName = "A. Bonmascota";
      }
      let name = previousValidName || "Unknown";
      let cleanedDescription = rawDescription.replace(/^\S+\s+/, "").replace(/\s*\d+([\s,]\d+)*$/g, "").trim();
      return {
        name,
        code: rawCode,
...
const syncController = {
  async syncWith1C(ctx) {
    try {
      ...
      const message = await strapi.plugin("vetgroup-product").service("syncService").syncItems();
      ctx.send({ message });
    } catch (err) {
...
const syncService = ({ strapi: strapi2 }) => ({
  async syncItems() {
    ...
    let createdCount = 0;
    let updatedCount = 0;
    const categories = await strapi2.db.query("api::category.category").findMany({ select: ["id", "title"] });
    for (const item of items) {
      const catalogName = item.CatalogName || "";
      const matchedCategory = categories.find(
        (cat) => catalogName.toLowerCase().includes(cat.title.toLowerCase())
      );
      ...
      const payload = {
        ...
        pack_price: item.pack_price ? parseInt(item.pack_price.replace(",", ".")) : 0
      };
      const existing = await strapi2.db.query("api::product.product").findOne({ where: { backendId: item.ID } });
      if (existing) {
        await strapi2.entityService.update("api::product.product", existing.id, {
          data: payload
        });
        updatedCount++;
      } else {
        await strapi2.entityService.create("api::product.product", {
          data: payload
        });
        createdCount++;
      }
...
console.log("💡 VetGroup Plugin Loaded");
const index = {
  register,
  bootstrap,
  destroy,
